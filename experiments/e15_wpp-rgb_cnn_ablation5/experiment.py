...
class Experiment15(Experiment):
    """CNN model predicting full ProMP weights of WPP using the WPP-RGB dataset.

    Task: WPP

    Dataset: WPP-RGB (Palpation paths: training/validation: 1, 2, 3, 6, 7, mixed training/validation and testing: 4, 5)
...
if __name__ == "__main__":
    ...
    parser = ExperimentArgumentParser("Experiment CNN WPP-RGB (Ablation 1): CNN model predicting full ProMP weights of WPP using the WPP-RGB dataset."
                                      "Palpation paths: training/validation: 1, 2, 3, 6, 7, mixed training/validation and testing: 4, 5")
    args = parser.parse_args()

    cfg = ConfigExp(args.name)
    exp = Experiment15(cfg)
