...
from utils.data import (get_train_val_test_idx_by_config_and_palp_path,
...
class Experiment17(Experiment):
    """CNN model predicting full ProMP weights of WPP using the WPP-RGB dataset.

    Task: WPP

    Dataset: WPP-RGB (Palpation paths: training/validation: 1, 4, 5; mixed training/validation and testing: 2, 3; excluded: 6, 7)
    ...
    def __init__(self, cfg: Config):
        ...
        # Fraction of the sample from each palpation path that will be used for testing.
        # The remaining samples will be used for training and validation.
        # If a palpation path is None then it's completely discarded (i.e. not used for training nor for testing).
        test_fractions = {
            "1": 0,
            "2": 0.5,
            "3": 0.5,
        ...
        self.idx_train, self.idx_val, self.idx_test = get_train_val_test_idx_by_config_and_palp_path(
            config_list, palp_path_list, test_fractions,
...
if __name__ == "__main__":
    ...
    parser = ExperimentArgumentParser("Experiment CNN WPP-RGB (Ablation 1): CNN model predicting full ProMP weights of WPP using the WPP-RGB dataset."
                                      "Palpation paths: training/validation: 1, 4, 5; mixed training/validation and testing: 2, 3; excluded: 6, 7")
    args = parser.parse_args()

    cfg = ConfigExp(args.name)
    exp = Experiment17(cfg)
