...
from utils.data import get_valid_ids, load_array_from_json, load_encoded_images
...
from utils.metrics import (EuclideanDistanceEePosFinalPromp,
                           ...
                           RmseJointsPromp)
from utils.models import get_encoder_model, get_fully_connected_model
...
class Experiment01(Experiment):
    """Fully connected model predicting full ProMP weights of RTP using the RTP-RGB dataset.
    ...
    Model: RGB image -> Encoder -> bottleneck image -> FC -> ProMP weights
    ...
    def __init__(self, cfg: Config):
        ...
        # Load samples ID.
        id_list = get_valid_ids([self.cfg.IMAGE_DIR, self.cfg.TRAJ_DIR])
        n_samples = id_list.shape[0]
        # Manually defined test samples for each region.
        self.idx_test = {
        ...
        # Unique regions.
        self.regions = list(sorted(set(self.idx_test.keys())))
        # Remove test samples from pool of samples.
        idx_train_val = list(range(n_samples))
        for idx_test_region in self.idx_test.values():
            for idx in idx_test_region:
                idx_train_val.remove(idx)

        self.idx_train, self.idx_val = train_test_split(idx_train_val, test_size=self.cfg.VAL_FRAC, random_state=self.cfg.RANDOM_STATE, shuffle=True)
...
if __name__ == "__main__":
    ...
    parser = ExperimentArgumentParser("Experiment FC RTP-RGB: Fully connected model predicting full ProMP weights of RTP using the RTP-RGB dataset.")
    args = parser.parse_args()

    cfg = ConfigExp(args.name)
    exp = Experiment01(cfg)
