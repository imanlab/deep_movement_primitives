...
from utils.metrics import (EuclideanDistanceEePosFinalPromp,
                           RmseEeOriAnglePromp, RmseEePosPromp,
                           RmseJointsPromp)
from utils.models import (get_convolutional_model, get_encoder_model,
                          get_residual_model)
from utils.output import (CustomFitOutput, ensure_dirs, evaluate_metrics,
                          plot_2d_traj, plot_3d_traj, plot_joint_traj,
                          plot_metric, plot_task_traj)
from utils.promp import ProMP


class Experiment05(Experiment):
    ...
    def __init__(self, cfg: Config):
        ...
        self.promp_weights_full = np.zeros((len(trajectories), self.promp.n_basis * self.promp.n_dof))
        for i, trajectory in enumerate(trajectories):
            self.promp_weights_full[i, :] = self.promp.weights_from_trajectory(trajectory)

        # Compute average ProMP weights on training dataset for each region.
        promp_weights_mean_by_region = {}
        for region in self.regions:
            region_idx = np.where(region_list[self.idx_train] == region)[0]
            promp_weights_mean_by_region[region] = np.mean(self.promp_weights_full[self.idx_train][region_idx], axis=0)

        # Assign the correct average ProMP weights to all samples.
        self.promp_weights_mean = np.zeros_like(self.promp_weights_full)
        for i, region in enumerate(region_list):
            self.promp_weights_mean[i, :] = promp_weights_mean_by_region[region]
    ...
    def get_model(self) -> tf.keras.Model:
        ...
        model = get_convolutional_model(
            output_size=self.promp.n_dof * self.promp.n_basis,
            activation=self.cfg.ACTIVATION, l1_reg=self.cfg.L1_REG, l2_reg=self.cfg.L2_REG
        )
        model = get_residual_model(model)
    ...
    def get_data(self, indices: np.ndarray) -> tuple[tuple[np.ndarray, np.ndarray], np.ndarray]:
        ...
        """
        X = (self.encoded_images[indices], self.promp_weights_mean[indices])
        y = self.promp_weights_full[indices]
...
if __name__ == "__main__":
    ...
    parser = ExperimentArgumentParser("Experiment CNN-RES RTP-RGBD: CNN model predicting residual ProMP weights of RTP using the RTP-RGBD dataset, "
                                      "averages computed for each region.")
    args = parser.parse_args()

    cfg = ConfigExp(args.name)
    exp = Experiment05(cfg)
