...
class Experiment13(Experiment):
    """CNN model predicting full ProMP weights of WPP using the WPP-RGB dataset.

    Task: WPP

    Dataset: WPP-RGB (Palpation paths: testing 3, 4; training/validation: 1, 2, 5; excluded: 6, 7)
...
if __name__ == "__main__":
    ...
    parser = ExperimentArgumentParser("Experiment CNN WPP-RGB (Ablation 1): CNN model predicting full ProMP weights of WPP using the WPP-RGB dataset."
                                      "Palpation paths: testing: 3, 4; training/validation: 1, 2, 5; excluded: 6, 7")
    args = parser.parse_args()

    cfg = ConfigExp(args.name)
    exp = Experiment13(cfg)
